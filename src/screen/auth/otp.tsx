  ...
  const handleOtpVerification = async () => {
    ...
    try {
      ...
      if (phoneNumber === STATIC_MOBILE_NUMBER && otp === STATIC_OTP) {
        console.log(
          'Static OTP verification detected - bypassing API and going directly to home',
        );
      ...
      console.log('OTP Response', response.data?.message?.user);
      ...
      if (response.data?.message?.message === 'Login successful!') {
        ...
        PrefManager.setValue(STRING.TOKEN, response.data?.message?.user?.token);
        PrefManager.setValue('userData', response.data?.message?.user);
        dispatch(
          setAuthToken({
            accessToken: response.data?.message?.user?.token,
            userData: response.data?.message?.user,
          }),
        );
      } else {
