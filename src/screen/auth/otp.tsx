...
const OtpScreen = ({ route, navigation }: any) => {
  ...
  const loginData: RegisterResponseModel = useSelector(
  ...
  const handleOtpVerification = async () => {
    ...
    try {
      ...
      if (response.status === 200) {
        ...
        const isProfileSubmit = userData?.isProfileComplete;
  ...
  const handleResendOtp = async () => {
    ...
    try {
      ...
      const response = await dispatch(resendOTPAction(payload));
        ...
        <View style={OtpScreenStyles.container}>
          ...
          <View style={OtpScreenStyles.mobileContainer}>
            <Text style={OtpScreenStyles.mobileText}>
              {loginData?.countryCode} {loginData?.mobileNumber}
            </Text>
